// src/app/mars/MarsMapContent.tsx
"use client";
...
const API_BASE_URL =
  ...
  "https://nasaspaceappchallenge-ijnb.onrender.com";
...
export default function MercuryMapContent() {
  ...
  const calculateDistance = (latlngs: LatLng[]): number => {
    const R = 2440; // Mercury radius in km
    let total = 0;
  ...
  const submitQuestion = async () => {
    ...
    const coordinates = [centroid.lng, centroid.lat];
  ...
  useEffect(() => {
    (window as any).expandQuestion = async (postId: string) => {
      ...
      try {
        ...
        const data = await res.json();
        setPostDetails({ post: data.post, comments: data.comments || [] });
        setCurrentPostId(id);
